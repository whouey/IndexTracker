...
INSERT INTO derivative (index_id, name, derivative_type)
    SELECT DISTINCT i.id, stg.contract, 'ETF' 
...
INSERT INTO derivative_detail (derivative_id)
    ...
    FROM table_name stg JOIN (SELECT * FROM derivative WHERE derivative_type='ETF') d ON d.name = stg.contract 
ON CONFLICT DO NOTHING;
...
INSERT INTO derivative_detail_history
    ...
    FROM table_name stg NATURAL JOIN (
        SELECT dd.id AS id, d.name AS contract FROM derivative_detail dd JOIN derivative d ON dd.derivative_id = d.id WHERE d.derivative_type='ETF') dev
