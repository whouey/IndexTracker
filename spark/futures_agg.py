...
if __name__ == "__main__":
    ...
    df = spark.read.parquet(f's3a://indextracker/tw/futures/trn/{ds}')
    
    scales = [1, 5, 15, 30, 60]

    roundup_to_minutes_udf = udf(roundup_to_minutes, TimestampType())

    window = Window.partitionBy(['contract', 'expire', 'scale', 'scale_time']).orderBy('datetime')
    window_unbound = Window.partitionBy(['contract', 'expire', 'scale', 'scale_time']).orderBy('datetime') \
    ...
    df.coalesce(1).write.parquet(f's3a://indextracker/tw/futures/agg/{ds}')
