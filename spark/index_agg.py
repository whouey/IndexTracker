...
if __name__ == "__main__":
    ...
    df = df.withColumn('scale', array([lit(x) for x in scales])) \
            ...
            .withColumn('open', first(col('index')).over(window_unbound)) \
            .withColumn('high', max(col('index')).over(window_unbound)) \
            .withColumn('low', min(col('index')).over(window_unbound)) \
            .withColumn('close', last(col('index')).over(window_unbound)) \
            .withColumn('mean', avg(col('index')).over(window_unbound)) \
            .withColumn('std', stddev(col('index')).over(window_unbound)) \
            .where(col('row') == 1) \
            .select(col('scale'), col('scale_time').alias('datetime'), \
                    col('open'), col('high'), col('low'), col('close'), col('mean'), col('std')) \
             
    # for every batch is very small in size, reduce the partitions to reduce the overhead 
    df.coalesce(1).write.parquet(f's3a://indextracker/tw/index/agg/{ds}')
