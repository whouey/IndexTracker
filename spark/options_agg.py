...
if __name__ == "__main__":
    ...
    window = Window.partitionBy(['contract', 'expire', 'strike_price', 'type', 'scale', 'scale_time']).orderBy('datetime')
    window_unbound = Window.partitionBy(['contract', 'expire', 'strike_price', 'type', 'scale', 'scale_time']).orderBy('datetime') \
    ...
    df = df.withColumn('scale', array([lit(x) for x in scales])) \
            ...
            .where(col('row') == 1) \
            .select(col('contract'), col('expire'), col('expire_code'), col('strike_price'), col('type'), col('scale'), col('scale_time').alias('datetime'), \
    ...
    df.coalesce(1).write.parquet(f's3a://indextracker/tw/options/agg/{ds}')
