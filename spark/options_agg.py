...
if __name__ == "__main__":
    ...
    df = spark.read.parquet(f's3a://indextracker/tw/options/trn/{ds}')
    
    scales = [1, 5, 15, 30, 60]

    roundup_to_minutes_udf = udf(roundup_to_minutes, TimestampType())

    window = Window.partitionBy(['contract', 'expire', 'strike_price', 'option_type', 'scale', 'scale_time']).orderBy('datetime')
    window_unbound = Window.partitionBy(['contract', 'expire', 'strike_price', 'option_type', 'scale', 'scale_time']).orderBy('datetime') \
    ...
    df = df.withColumn('scale', array([lit(x) for x in scales])) \
            ...
            .where(col('row') == 1) \
            .select(col('contract'), col('expire'), col('expire_code'), col('strike_price'), col('option_type'), col('scale'), col('scale_time').alias('datetime'), \
    ...
    df.coalesce(1).write.mode('overwrite').parquet(f's3a://indextracker/tw/options/agg/{ds}')
